...
class ChainfeedsSpider(scrapy.Spider):
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        ...
        spider.redis_url_key = spider.settings.get('REDIS_URL_KEY', 'chainfeeds-url')
    ...
    def parse_article(self, response):
        ...
        item['author'] = "Chainfeeds投研早报"
        # item['content'] = '\n'.join(response.css('div.post-content p').xpath('text()').getall())  # 这里如果不需要原来的content提取方式可以注释掉
        print(item)
        yield item